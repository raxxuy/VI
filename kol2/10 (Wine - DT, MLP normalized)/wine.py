...
if __name__ == '__main__':
    ...
    # modify data
    train_x = [row[:least_important] + row[least_important + 1:] for row in train_x]
    test_x = [row[:least_important] + row[least_important + 1:] for row in test_x]
    ...
    print(f'Tochnost so StandardScaler: {classifier.score(standard_scaler.transform(test_x), test_y)}')

    classifier.fit(min_max_scaler.transform(train_x), train_y)
    print(f'Tochnost so MinMaxScaler: {classifier.score(min_max_scaler.transform(test_x), test_y)}')
