...
dataset = [
    [30, 92726, 2.0, 9, 2011, -37.8497, 144.968, 10, 0],
...
if __name__ == '__main__':
    ...
    train_x_reduced = [row[:index] + row[index + 1:] for row in train_x]
    ...
    classifier = MLPClassifier(neurons, activation='relu', max_iter=20, learning_rate_init=learning_rate, random_state=0)
    classifier.fit(scaler.transform(train_x), train_y)

    # accuracy
    accuracy1, accuracy2 = 0, 0

    for prediction, true in zip(classifier.predict(scaler.transform(train_x)), train_y):
        if prediction == true:
            accuracy1 += 1

    for prediction, true in zip(classifier.predict(scaler.transform(val_x)), val_y):
        if prediction == true:
            accuracy2 += 1

    accuracy1 /= len(train_set)
    accuracy2 /= len(val_set)

    # input sample
    sample = [list(map(float, input().split(" ")))]
    sample_reduced = [row[:index] + row[index + 1:] for row in sample]
    ...
    if accuracy1 > accuracy2 * 1.15:
        ...
        classifier.fit(scaler.transform(train_x_reduced), train_y)

        print(classifier.predict(scaler.transform(sample_reduced))[0])
    else:
