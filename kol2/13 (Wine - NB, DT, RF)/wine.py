...
if __name__ == '__main__':
    ...
    twos = [row for row in dataset if row[-1] == 2]

    # train data
    train_set1 = zeros[:int(x1 * len(zeros))] + ones[:int(x1 * len(ones))] + twos[:int(x1 * len(twos))]
    train_x1 = [row[:-1] for row in train_set1]
    train_y1 = [row[-1] for row in train_set1]

    train_set2 = zeros[int(x1 * len(zeros)):int(x2 * len(zeros))] + ones[int(x1 * len(ones)):int(x2 * len(ones))] + twos[int(x1 * len(twos)):int(x2 * len(twos))]
    train_x2 = [row[:-1] for row in train_set2]
    train_y2 = [row[-1] for row in train_set2]

    train_set3 = zeros[:int(x2 * len(zeros))] + ones[:int(x2 * len(ones))] + twos[:int(x2 * len(twos))]
    train_x3 = [row[:-1] for row in train_set3]
    train_y3 = [row[-1] for row in train_set3]

    # test data
    test_set = zeros[int(x2 * len(zeros)):] + ones[int(x2 * len(ones)):] + twos[int(x2 * len(twos)):]
    ...
    for x, true in zip(test_x, test_y):
        predictions = [classifier1.predict([x])[0], classifier2.predict([x])[0], classifier3.predict([x])[0]]

        if predictions.count(true) >= 2:
            accuracy += 1

    # results
    print(f'Tochnost: {accuracy / len(test_set)}')
